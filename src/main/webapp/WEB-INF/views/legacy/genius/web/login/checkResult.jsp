<%--
  Migrated from genius: WebContent/WEB-INF/jsp/web/login/checkResult.jsp
...
<script type="text/javascript">
    ...
    CtlExecutor = {
        doLoginProc: function () {
            ...
            $.ajax({
                url: loginEndpoint("cdsLoginProcAjax"),
                async: false,
                type: 'POST',
                timeout: 3000,
                dataType: "json",
                    ...
                    if (data.result == 'N') {
                        ...
                        /* if(data.sessionDupChk === true) {
                            if(confirm("현재 다른 기기에서 로그인되어 있습니다.\n기존 세션을 종료하고 이 기기에서 로그인하시겠습니까?")) {
                                $.ajax({
                                    url : loginEndpoint('forceLoginAjax'),
                                    async : false,
                                    type : 'POST',
                                    timeout : 3000,
                                    dataType : "json",
