...
- 본체는 **User-Agent 기반 판별**(phone/tablet/desktop) — JWT와 무관한 요청 헤더 파싱이다.
...
| 신호 | 대체 |
|---|---|
| phone/tablet/desktop | 요청마다 `User-Agent` 파싱하는 유틸 (JWT 아님) |
...
```java
public enum DeviceType { WEB, MOBILE, TABLET }

@Component
public class DeviceResolver {
    private static final Pattern TABLET = Pattern.compile("(?i)(ipad|android(?!.*mobile))");
    private static final Pattern MOBILE = Pattern.compile("(?i)(iphone|ipod|android.*mobile|windows phone|blackberry)");

    public DeviceType resolve(HttpServletRequest request) {
        // 네이티브 앱 웹뷰 → 레거시 appCheck/isDevice 대체 (앱이 헤더를 보냄)
        if ("app".equalsIgnoreCase(request.getHeader("X-Client-Type"))) return DeviceType.MOBILE;

        String ua = request.getHeader("User-Agent");
        if (ua == null) return DeviceType.WEB;
        if (TABLET.matcher(ua).find()) return DeviceType.TABLET;
        if (MOBILE.matcher(ua).find()) return DeviceType.MOBILE;
...
- **`preUrl`**(레거시: 앱/모바일이면 `/mobile/m`) → 컨트롤러에서 `DeviceResolver.resolve(request)` 결과로 뷰 prefix 결정, 또는 뷰에 노출: `CurrentUserModelAdvice` 처럼 `@ControllerAdvice`에서 `@ModelAttribute("deviceType")`로 내려주고 JSP에서 `${deviceType}` 사용.
- **로깅 디바이스(W/M/T)** → 로깅을 남긴다면 같은 유틸 결과 사용.
- **모바일/웹 JSP 분기** → 컨트롤러가 `deviceType`에 따라 뷰 이름을 고르거나, 뷰 안에서 `${deviceType}`로 분기.

> 모바일 사이트를 신규에서 유지하지 않기로 하면 이 전체가 불필요하다. 유지 여부부터 확정하고 진행한다.
