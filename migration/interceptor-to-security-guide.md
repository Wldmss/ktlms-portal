...
## 1.5 결국 남는 작업 (로깅/접속통계까지 이관 안 하기로 한 경우)

(a) 세션 체크는 삭제, (c) 디바이스·loginUser는 완료, (d) 로깅 AOP·접속통계는 이관 안 함, (e)는 삭제로 정리하면 **실제로 남는 "새 작업"의 대부분은 (b) 메뉴/URL 권한 → `@PreAuthorize`** 다.

단, **메뉴 권한과 무관한데 그냥 사라지지 않는 2가지**를 놓치면 안 된다:

1. **중복 로그인(단일 세션) 정책** — genius `MicroSessionCheckInterceptor`, lms `SessionCheckInterceptor`의 세션ID 대조 부분. 메뉴 권한도 아니고 삭제도 아니다. STATELESS/JWT 에서는 **refresh token 단일세션 정책**(로그인 시 해당 userId 의 기존 refresh token 무효화)으로 다시 설계해야 하는 **인증 정책 항목**. 상세 6절, [session-to-jwt-guide.md](session-to-jwt-guide.md) 8절 연계.
2. **`adminAuthList` 뷰 노출** — 두 프로젝트 SessionCheck 의 `postHandle`이 관리자 메뉴/버튼 노출용 권한 목록을 request 에 실어주던 (c) 부분. **메뉴 권한(b) 작업과 한 몸**으로 처리한다(권한 조회 → 뷰에 전달).

그리고 조건부(해당 메뉴가 딸려올 때만): **CourseInterceptor**(학습 로그) → course 메뉴 이관 시, **접속통계** → 그 리포트 메뉴가 실제 살아있을 때만.
